...
export default function ChatPanel({ roomId, partnerName = "Stranger" }) {
  ...
  return (
    <div className="flex flex-col h-full bg-panel rounded-xl border border-white/10 overflow-hidden surface-lift animate-enter">
      ...
      {/* FIX: Added overscroll-contain to prevent mobile browser bouncing */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto overscroll-contain px-4 py-3 space-y-2 min-h-0">
        {messages.length === 0 && (
          <p className="text-mist/60 text-sm font-body text-center mt-2">
            Say hi 👋 — messages stay in this session only.
          </p>
        )}
        {messages.map((m, i) => (
          <div key={i} className={`max-w-[85%] sm:max-w-[80%] ${m.from === "me" ? "ml-auto" : "mr-auto"}`}>
        ...
        {partnerTyping && (
          <p className="text-xs text-mist font-mono animate-pulse">{partnerName} is typing…</p>
        )}
      </div>

      <div className="p-2 sm:p-3 border-t border-white/5 flex gap-2 shrink-0">
        <input
          value={draft}
          onChange={(e) => {
          ...
          maxLength={2000}
          className="flex-1 bg-panel2 rounded-lg px-3 py-2 text-sm text-white placeholder:text-mist/50 outline-none focus-visible:outline-signal focus-visible:ring-1 focus-visible:ring-signal min-h-[44px]"
        />
        <button
          onClick={send}
