      ...
      {!game && <div className="mt-4">
        ...
        {isModerator ? <>
          <div className="mt-4 grid grid-cols-2 gap-2">
            <button onClick={() => setMode("pulse")} className={`ui-button min-h-[4.5rem] rounded-xl border px-3 py-3 text-left ${mode === "pulse" ? "border-fuchsia-300/60 bg-fuchsia-400/15" : "border-white/10 bg-black/10 hover:bg-white/5"}`}>
            ...
            <button onClick={() => setMode("draw")} className={`ui-button min-h-[4.5rem] rounded-xl border px-3 py-3 text-left ${mode === "draw" ? "border-signal/70 bg-signal/15" : "border-white/10 bg-black/10 hover:bg-white/5"}`}>
            ...
            <button onClick={() => setMode("bomb")} className={`ui-button min-h-[4.5rem] rounded-xl border px-3 py-3 text-left ${mode === "bomb" ? "border-amber-300/70 bg-amber-300/15" : "border-white/10 bg-black/10 hover:bg-white/5"}`}>
            ...
            <button onClick={() => setMode("uno")} className={`ui-button min-h-[4.5rem] rounded-xl border px-3 py-3 text-left ${mode === "uno" ? "border-emerald-300/70 bg-emerald-300/15" : "border-white/10 bg-black/10 hover:bg-white/5"}`}>
        ...
        <div className="mt-4 rounded-xl border border-white/10 bg-black/15 p-3 text-center">
          ...
          {question && <>
            ...
            <div className="mt-3 grid gap-2">
              {game.answers.map((answerOption) => <button key={answerOption} disabled={submittedToken === game.questionToken} onClick={() => answer(answerOption)} className="ui-button ui-button-violet min-h-11 justify-start rounded-xl px-3 text-left text-xs text-white disabled:opacity-60">{answerOption}</button>)}
            </div>
          ...
          {draw && game.status === "drawing" && <>
            ...
            {!isDrawer && <form onSubmit={submitGuess} className="mt-3 flex gap-2"><input value={guess} onChange={(event) => setGuess(event.target.value)} placeholder="Type your guess" className="min-w-0 flex-1 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-xs text-white outline-none" /><button className="ui-button ui-button-primary px-3 text-xs">Guess</button></form>}
          </>}
          ...
          {bomb && <>
            ...
            {isBombTurn ? <form onSubmit={submitGuess} className="mt-3 flex gap-2"><input autoFocus value={guess} onChange={(event) => setGuess(event.target.value)} placeholder="Type a word…" className="min-w-0 flex-1 rounded-lg border border-amber-300/20 bg-black/20 px-3 py-2 text-xs text-white outline-none" /><button className="ui-button bg-amber-300 px-3 text-xs font-bold text-ink">Pass 💥</button></form> : <p className="mt-3 rounded-lg bg-white/5 px-3 py-2 text-xs text-mist">{game.bombTurnName} has the bomb. Watch closely!</p>}
          </>}
