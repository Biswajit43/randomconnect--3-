...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        const [currentReports, pendingReports, roomData, usageData, banData, auditData, premiumInvitesData] = await Promise.all([
          api.adminReports(status), api.adminReports("pending"), api.adminRooms(), api.adminUsage(), api.adminBans(), api.adminAudit(), api.adminPremiumInvites(),
        ]);
        if (!active) return;
        setReports(currentReports); setPendingCount(usageData.pendingReports ?? pendingReports.length); setRooms(roomData); setUsage(usageData); setBans(banData); setAudit(auditData); setPremiumInvites(premiumInvitesData); setError("");
  ...
  async function logout() {
    ...
    setAuthenticated(false); setAdminRole(null); setReports([]); setRooms([]); setBans([]); setAudit([]); setPremiumInvites([]);
  }
